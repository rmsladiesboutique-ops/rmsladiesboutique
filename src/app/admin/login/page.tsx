...
export default function AdminLoginPage() {
  ...
  return (
    <main className="mx-auto flex min-h-screen max-w-6xl items-center px-4 py-12 sm:px-6 lg:px-8">
      <div className="grid w-full gap-8 rounded-[2rem] border border-black/10 bg-white/95 p-6 shadow-[0_30px_70px_-45px_rgba(15,15,15,0.3)] backdrop-blur-xl text-slate-950 dark:border-white/10 dark:bg-slate-950/95 dark:text-slate-100 md:grid-cols-[1.15fr_0.85fr] md:p-10 lg:p-12">
        ...
        <Card className="bg-white/95 text-slate-950 shadow-xl shadow-amber-500/10 dark:bg-slate-950/95 dark:text-white">
          <CardContent className="space-y-6">
            ...
            <div className="space-y-4">
              <label className="block text-sm font-medium text-slate-700 dark:text-slate-300">Admin ID</label>
              <Input autoComplete="username" placeholder="admin" value={id} onChange={(e) => setId(e.target.value)} className="border-slate-300 bg-slate-50 text-slate-950 focus:border-amber-500 dark:border-slate-700 dark:bg-slate-900/90 dark:text-white" />

              <label className="block text-sm font-medium text-slate-700 dark:text-slate-300">Password</label>
              <Input autoComplete="current-password" type="password" placeholder="••••••••" value={password} onChange={(e) => setPassword(e.target.value)} className="border-slate-300 bg-slate-50 text-slate-950 focus:border-amber-500 dark:border-slate-700 dark:bg-slate-900/90 dark:text-white" />
            ...
            <Button onClick={submit} className="w-full bg-amber-500 text-zinc-950 hover:bg-amber-400 dark:text-slate-950">Sign in securely</Button>
          </CardContent>
        </Card>
      </div>
    </main>
